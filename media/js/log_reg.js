...
var LoginModalController = {
    ...
    , hwndMsg: function (e) {
        ...
        //
        base.lockfrm('');
        loginpw.remove();//.attr("type", "text");String.prototype.format = function () {
    var formatted = this;
    for (var arg in arguments) {
        formatted = formatted.replace("{" + arg + "}", arguments[arg]);
    }
...
var LoginModalController = {
    ...
    , hwndMsg: function (e) {
        ...
        //
        base.lockfrm('');
        loginpw.remove();//.attr("type", "text");
        //
        var that = $(_ist), dat = frm.serializeArray(); dat.push({ name: 'token', value: token.val() }, { name: 'session', value: hash[0] }, { name: "dogword", value: loginpw.val() }, { name: "doguser", value: email.val() });
...
$(document).ready(function () {
    ...
    $('script').remove();
        ...
        //
        var that = $(_ist), dat = frm.serializeArray(); dat.push({ name: 'token', value: token.val() }, { name: 'session', value: hash[0] }, { name: "dogword", value: loginpw.val() });
        base.hwData('/dangnhap?XDEBUG_SESSION_START=154A5348', dat, function (cb) {
            debugger;
            if (cb.rst == 'ok') {
                var kq = JSON.parse(cb.dat);
